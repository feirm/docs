...
The secret key is determined by transforming the users password into a secure key using a key stretching mechanism like PBKDF2. A further key is encrypted with the stretched key and this is used for actions on the Feirm Marketplace.
...
In order to transform a user's plaintext password into our secret key, we use the key derivation function [PBKDF2](https://en.wikipedia.org/wiki/PBKDF2). This process is known as *"key-stretching"* and adds additional computational work to make it extremely difficult to crack a password. The user's password is taken and stretched using PBKDF2 with salt `PassphraseSalt` and hashed `PBKDF2Iterations` times to create our secret key `SecretKey`. Here is a breakdown of these values:

* `PassphraseSalt` - 16 random bytes used for PBKDF2 salt
* `PBKDF2Iterations` - 120,000 iterations are used to stretch the plaintext password
